...
import { saveCall } from '@/lib/callStore';
import type { ConversationState, DemoVendorParticipant, JobSpec } from '@/lib/types';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { vendor?: DemoVendorParticipant; job_spec?: JobSpec; mode?: 'quote' | 'negotiate'; leverage?: ConversationState['leverage'] };
    if (!body.vendor || !body.job_spec || !body.mode) return NextResponse.json({ error: 'vendor, job_spec, and mode are required' }, { status: 400 });
    if (!body.job_spec.confirmed_by_user || !body.job_spec.spec_hash) return NextResponse.json({ error: 'A confirmed immutable wedding brief is required before calling.' }, { status: 400 });
